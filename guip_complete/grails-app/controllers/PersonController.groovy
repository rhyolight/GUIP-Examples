...
class PersonController {
    ...
    def ajaxTable = {
        println params
        if (params.sort == 'name') params.sort = 'lastName'
        def people = Person.list(params)
        def peopleList = people.collect { person ->
            [
                name: person.firstName + ' ' + person.lastName,
                birthDate: grailsUITagLibService.dateToJs(person.birthDate),
                age: Math.floor((new Date().time - person.birthDate.time) / 31556926000),
